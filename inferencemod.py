    ...
    while True:
        ...
        if answers is None:
            ...
            for i, _ in enumerate(answers['scores']):
                arr.append("{}- {}{} %[{}]% {}{}{}".format(colorama.Fore.GREEN if answers['scores'][i] == max(answers['scores']) and answers['scores'][i] >= score_settings['bad_response_threshold'] else colorama.Fore.YELLOW if answers['scores'][i] >= score_settings['bad_response_threshold'] else colorama.Fore.RED, answers['answers'][i], colorama.Fore.RESET, answers['scores'][i], colorama.Fore.BLUE, answers['score_modifiers'][i] if score_settings['show_score_modifiers'] else '', colorama.Fore.RESET))
            for xx in arr:
                content=xx.split('%')
                res=content[0]
                result=res.find('https:')
                if result != -1:
                    continue
                weight=str(content[1])
                weight = weight.replace('[','')
                weight=weight.replace(']','')
                weight=float(weight)
                max_weight.append(weight)
                dico[res]=weight
            maxx_weight=sorted(max_weight,reverse = True)
            maxx=maxx_weight[0]
            for i in dico:
                if dico[i]==maxx:
                    probable_output.append(i)
            print(random.choice(probable_output))
os.chdir(original_cwd)
